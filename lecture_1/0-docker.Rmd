...
```
docker run -d -p 8787:8787 -p 8888:8888 nanounanue/docker-ds
```
...
```
docker run --rm -it --user itam --name r-console -v path_a_carpeta:/home/itam nanounanue/docker-ds /usr/bin/R
```

Lanzar la terminal de `python` con las librerías de cómputo científico:

```
docker run --rm -it --user itam --name ipython-console -v path_a_carpeta:/home/itam nanounanue/docker-ds /usr/bin/ipython
```

O un shell simple (en este caso `zsh`)

```
docker run --rm -it --user itam  --name commandline -v path_a_carpeta:/home/itam nanounanue/docker-ds /bin/zsh
```
