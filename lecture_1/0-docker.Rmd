...
```
docker start -a -d stoic_hopper # Esto funciona si ejecutaste la versión de demonio
```
...
```
docker run --rm -it --user itam nanounanue/docker-ds /usr/bin/R
```

Lanzar la terminal de `python` con las librerías de cómputo científico:

```
docker run --rm -it --user itam nanounanue/docker-ds /usr/bin/ipython
```

O un shell simple (en este caso `zsh`)

```
docker run --rm -it --user itam  nanounanue/docker-ds /bin/zsh
```
